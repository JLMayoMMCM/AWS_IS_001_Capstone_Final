...
**Schema source of truth:** `0001_initial_schema.sql`
...
|------|--------|----------|
| `/` (welcome) | Public | SSR fetch of `site_settings` (single row) + latest 3 approved public blogs + next 3 upcoming approved public events from RDS. |
...
| members | Profile data. Linked 1:1 to users via user_id. References lookup tables for course, year_level, and status. |
| courses | Admin-managed lookup of degree programs. Referenced by `members.course_id`. |
| year_levels | Admin-managed lookup. Seeded with Year 1–4. Referenced by `members.year_level_id`. |
| member_statuses | Admin-managed lookup. Seeded with Active, For Renewal, Inactive, Alumni. Referenced by `members.status_id`. |
| officers | Officer roles. Linked to members and officer_positions; multiple officers per term; holds term, order. |
| officer_positions | Lookup table of officer position titles AND which 3 positions are required to approve events (`is_approver` flag, enforced by trigger). |
| blogs | Blog posts. Holds title, slug, body, author_id, status (draft/pending/approved/rejected), visibility (public/private). |
| blog_attachments | Per-blog extra images, external links, or resource_link references. |
...
| projects | Member projects. Holds title, description, repo_url, live_url, category_id, status, visibility, dates. |
| project_categories | Admin-managed lookup. Seeded with Web, Mobile, Cloud, Data & AI, Hardware, Research. Referenced by `projects.category_id`. |
| project_contributors | Many-to-many join: which members contributed to which project, with their role on it. |
| project_attachments | Per-project extra images or external links. |
| events | Org events. Holds title, description, location, start/end datetime, status, visibility, created_by (officer). |
| event_approvals | Records each approval/rejection vote on an event by a required officer position. Validated and finalized by triggers. |
| form_links | External form integrations. References form_providers for the provider, holds url and derived embed_url, visibility, order. |
| form_providers | Admin-managed lookup. Seeded with Google, Microsoft. Referenced by `form_links.provider_id`. |
| site_settings | Single-row CMS for org profile: org_name, short_name, tagline, about (paragraphs), term, contact_email, contact_address, contact_hours. |
| audit_log | Append-only record of admin actions. UPDATE and DELETE denied by trigger (DR-07). |
...
This is the implemented schema as of v3 of this document. It matches `0001_initial_schema.sql` verbatim and is the canonical source of truth — any future change goes there first, then is reflected here. The schema is grouped into: extensions, enums, the `set_updated_at` trigger helper, admin-managed lookup tables, the core entity tables, and the audit/trigger machinery that enforces multi-officer event approval and audit-log immutability.
...
- Vocabulary tables (`courses`, `year_levels`, `member_statuses`, `project_categories`, `form_providers`) replaced what used to be ENUMs in earlier drafts. Tables let admins add or rename values from the Categories page without a schema migration — important for a system handed off between officer teams each year.
- All FKs from `members` and `projects` into these vocabularies use `ON DELETE SET NULL` — removing a vocabulary value clears the references rather than blocking the delete or cascading destruction.
...
- Event approval rules (validate-the-voter, finalize-the-status, deny-on-rejection) live in BEFORE/AFTER INSERT triggers. The API just inserts one row — the database guarantees correctness even if the API has bugs.
...
- `audit_log` is append-only with JSONB before/after — one table satisfies the entire audit-trail requirement. A BEFORE UPDATE/DELETE trigger denies modification at the database level, not just at the application role.
- `site_settings` is a single-row table (BOOLEAN PK with `CHECK (id)`) — exactly the right shape for org-profile data that has one and only one valid value at any time.
