...
| 1.1 | 2026-05-22 | Tech Committee | MVP implemented. Build decisions, added capabilities, and the schema evolution are recorded in `Implementation_Notes.md`. |
...
| FR-OFF-05 | The system SHALL prevent an officer from voting on an event they themselves created. The API MUST return HTTP 403 with a clear error message. | System | T, I |
| FR-OFF-06 | When the third 'approved' vote is recorded, the system SHALL atomically (in the same transaction as the vote insert) set events.status = 'approved' and events.approved_at = NOW(). | System | T, I |
| FR-OFF-07 | Any single 'rejected' vote SHALL immediately set events.status = 'rejected'. The rejection comment SHALL be required (non-null, non-empty). | System | T |
...
| members | Profile data. 1:1 with users. |
| officer_positions | Lookup of position titles. Exactly 3 rows have is_approver = TRUE. |
...
| blog_attachments | Extra images and external links per blog. |
| projects | Member projects with repo / live links and contributor list. |
...
| event_approvals | One vote per required officer position. Unique on (event_id, position_id). |
| resources | Resource metadata; binary lives in S3. |
| resource_categories | Lookup table for resources. |
| form_links | External Google/Microsoft Forms embeds. |
| audit_log | Append-only record of all sensitive writes. |
...
| DR-03 | Email columns SHALL use the CITEXT case-insensitive type. | I |
...
| DR-06 | The officer_positions table SHALL be constrained (via trigger) such that exactly 3 rows have is_approver = TRUE. | T, I |
| DR-07 | The audit_log table SHALL deny UPDATE and DELETE to the application database role. | T, I |
